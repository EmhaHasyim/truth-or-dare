...
function RoomPage() {
  ...
  onMount(async () => {
    ...
    // Prefer the playerId from the room listing; fall back to the one stored
    // when this browser created/joined the room (needed for reconnects after
    // the player row was cleaned up from D1).
    const myPlayer = currentRoom.players.find((p: Player) => p.name === username)
    const storedSession = getStoredPlayerSession(id)
    const playerIdFromRoom = myPlayer?.id || storedSession?.playerId || ''
    // If this browser's username isn't in the room listing (e.g. the user
    // changed their stored name after joining), use the name we registered
    // with so the server's name-match check still passes.
    const wsName = myPlayer ? username : storedSession?.playerName || username
    connectWs(id, wsName, playerIdFromRoom)
  })
  ...
  function connectWs(roomId: string, name: string, playerIdFromRoom: string) {
    ...
    const url = playerIdFromRoom
      ? getWsUrl(
          `/ws/${roomId}?name=${encodeURIComponent(name)}&playerId=${encodeURIComponent(playerIdFromRoom)}`,
        )
  ...
  function scheduleReconnect(roomId: string, name: string, playerIdFromRoom: string) {
    ...
    if (!ws() || reconnectCount >= MAX_RECONNECT_ATTEMPTS) {
      if (reconnectCount >= MAX_RECONNECT_ATTEMPTS)
        setError('Koneksi terputus. Refresh halaman untuk mencoba lagi.')
      setIsReconnecting(false)
      return
    }
    reconnectCount++
  ...
  const username = getStoredUsername()
  // Host status is live via the WS player list (kept in sync after a host
  // transfer). The room().hostName check only covers the initial paint before
  // the first room_state arrives.
  const iAmHost = () =>
    room()!.hostName === username || players().find((p) => p.name === username)?.isHost === true
  const currentHostName = () => players().find((p) => p.isHost)?.name || room()!.hostName
          ...
          <div class="max-w-sm mx-auto w-full flex-1 flex flex-col">
              ...
              <div class="flex items-center gap-3 bg-warning/10 border border-warning/20 rounded-2xl px-4 py-3 mb-4">
                ...
                <div>
                  ...
                  <p class="text-xs text-base-content/50">
                    Percobaan {reconnectCount}/{MAX_RECONNECT_ATTEMPTS}
                  </p>
                </div>
              </div>
            </Show>
            ...
            <div class="bg-base-100 rounded-2xl shadow-sm border border-base-200 p-5 mb-4">
              <div class="flex items-start justify-between gap-3 mb-4">
                <div class="min-w-0">
                  ...
                  <p class="text-xs text-base-content/50 mt-0.5">
                    Oleh {currentHostName()} · {players().length}/{room()!.maxPlayers} pemain
                  </p>
                </div>
                <button
                  class="btn btn-ghost btn-sm min-h-[36px] rounded-xl"
                  aria-label="Kembali"
                  onClick={() => navigate({ to: '/' })}
                >
                  <ArrowLeft size={16} />
              ...
              <div class="bg-base-200 rounded-xl p-4 text-center mb-4">
                ...
                <button
                  class="btn btn-ghost btn-sm min-h-[36px] rounded-xl mt-2 text-xs"
                  onClick={() => {
              ...
              <div class="flex gap-2">
                <button
                  class="btn btn-outline flex-1 min-h-[44px] rounded-xl text-xs font-medium"
                  onClick={() => {
                ...
                <button
                  ...
                  onClick={() =>
                    window.open(
                      `https://wa.me/?text=${encodeURIComponent('Ikutan main Truth or Dare yuk! Kode: ' + room()!.code)}`,
                    )
                  }
                >
            ...
            <div class="bg-base-100 rounded-2xl shadow-sm border border-base-200 p-5 flex-1">
              ...
              <ul class="space-y-2">
                {players().map((p, idx) => (
                  <li
                    class="flex items-center gap-3 bg-base-200/70 rounded-xl px-4 py-3 phase-fade-in"
            ...
            <div class="mt-4 space-y-2.5">
              ...
              <button
                class="btn btn-ghost w-full min-h-[44px] rounded-xl text-sm text-base-content/50"
                onClick={() => navigate({ to: '/' })}
              >
